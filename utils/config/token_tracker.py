...
class TokenUsageTracker:
    ...
    def __init__(self, storage_path: str = None):
        ...
        Args:
            storage_path: Path to store token usage data. If None, uses default path.
        """
    ...
    def record_usage(self, model: str, tokens: int, request_type: str = 'completion') -> None:
        ...
        Args:
            ...
            request_type: The type of request (completion, embedding, etc.)
        """
        if not self.track_usage:
            return
    ...
    def _check_limits(self) -> None:
        ...
        try:
            ...
            if daily_percent >= self.alert_percent:
                logger.warning(f"Daily token usage at {daily_percent:.2f}% of limit ({self.usage_data['daily_tokens']}/{self.daily_limit})")
                ...
                try:
                    ...
                    alert_manager.send_alert(
                        "llm_token_limit_approaching",
                        f"Daily token usage at {daily_percent:.2f}% of limit ({self.usage_data['daily_tokens']}/{self.daily_limit})",
                        "warning"
                    )
            ...
            if monthly_percent >= self.alert_percent:
                logger.warning(f"Monthly token usage at {monthly_percent:.2f}% of limit ({self.usage_data['monthly_tokens']}/{self.monthly_limit})")
                ...
                try:
                    ...
                    alert_manager.send_alert(
                        "llm_token_limit_approaching",
                        f"Monthly token usage at {monthly_percent:.2f}% of limit ({self.usage_data['monthly_tokens']}/{self.monthly_limit})",
                        "warning"
                    )
                except Exception as alert_error:
                    pass  # Continue even if alert fails
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        if not self.track_usage:
            return {
                ...
                'message': 'Token usage tracking is disabled'
            }
